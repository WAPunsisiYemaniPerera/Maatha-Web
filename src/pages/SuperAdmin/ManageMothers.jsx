    ...
    <AdminLayout>
      ...
      {message && (
        <div className={`fixed top-5 right-5 z-[130] text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center space-x-4 animate-in slide-in-from-right duration-500 ${
          messageType === 'error' ? 'bg-red-900 border-l-4 border-red-500' : 'bg-slate-900 border-l-4 border-rose-500'
        }`}>
          <div className={`rounded-full p-1.5 ${messageType === 'error' ? 'bg-red-500' : 'bg-rose-500'}`}>
      ...
      {viewingMother && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl max-w-3xl w-full shadow-2xl overflow-hidden border border-gray-100 animate-in zoom-in-95 duration-300">
            {/* Modal Header */}
            <div className={`p-6 text-white relative ${
              viewingMother.riskStatus === 'High-Risk' 
                ? 'bg-gradient-to-r from-red-700 via-rose-800 to-slate-900' 
                : 'bg-gradient-to-r from-rose-700 via-pink-800 to-slate-900'
            }`}>
              <button 
                ...
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {viewingMother.bloodGroup && (
                      <span className="bg-white/20 backdrop-blur-md px-2.5 py-0.5 rounded-full text-xs font-mono font-bold text-white border border-white/20">
                        🩸 {viewingMother.bloodGroup}
                      </span>
                    )}
                  </div>
                  <h3 className="text-2xl sm:text-3xl font-extrabold tracking-tight">{viewingMother.fullName}</h3>
            ...
            <div className="p-6 space-y-5 max-h-[72vh] overflow-y-auto custom-scrollbar">
              ...
              {(() => {
                ...
                return (
                  <div className="bg-gradient-to-br from-rose-50 to-pink-50/50 p-4 rounded-2xl border border-rose-100 space-y-2">
                    <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                      <span className="flex items-center gap-1.5 text-rose-800">
                        <span className="text-base">⏱️</span> ගර්භනී සති ප්‍රගතිය (Gestational Progress)
                      </span>
                      <span className="font-mono text-rose-700 bg-rose-100 px-2.5 py-0.5 rounded-lg text-xs font-black">
                        {weeks > 0 ? `${weeks} / 40 සති (${Math.round(progressPercent)}%)` : 'සති සටහන් වී නැත'}
                      </span>
                    </div>
                    <div className="w-full bg-rose-200/60 rounded-full h-3.5 overflow-hidden p-0.5">
                      <div 
                        className="bg-gradient-to-r from-rose-500 to-pink-600 h-full rounded-full transition-all duration-700 shadow-sm"
                        style={{ width: `${progressPercent}%` }}
                      ></div>
                    </div>
                    <div className="flex justify-between text-[10px] font-bold text-gray-500 pt-0.5">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                  ...
                  <span className="text-base font-black text-rose-600 mt-1 block font-mono">{viewingMother.bloodGroup || '—'}</span>
                </div>
              </div>
              ...
              <div className="bg-gray-50/70 p-4 rounded-2xl border border-gray-100 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  <div>
                    <span className="text-gray-400 font-bold block">අවදානම් තත්ත්වය:</span>
                  ...
                  <div>
                    <span className="text-gray-400 font-bold block">ගර්භය (Gravida / Parity):</span>
                    <span className="font-semibold text-gray-700 font-mono">
                      G: {viewingMother.gravida || '1'} | P: {viewingMother.para || '0'}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-400 font-bold block">ප්‍රසූතිය සිදු කෙරෙන රෝහල:</span>
                    <span className="font-semibold text-gray-700">{viewingMother.hospitalName || 'පවරා නැත (N/A)'}</span>
                  </div>
                  <div>
                    <span className="text-gray-400 font-bold block">විශේෂ සෞඛ්‍ය සටහන් (Notes):</span>
                    <span className="font-semibold text-gray-700">{viewingMother.notes || viewingMother.riskNotes || 'කිසිදු විශේෂ සටහනක් නැත'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50/70 p-4 rounded-2xl border border-gray-100 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  <div>
                    ...
                    {viewingMother.phone ? (
                      <a href={`tel:${viewingMother.phone}`} className="font-bold text-blue-600 hover:underline flex items-center gap-1">
                        📞 {viewingMother.phone}
                      </a>
                    ) : <span className="text-gray-400">නොදක්වා ඇත</span>}
                  </div>
                  <div>
                    <span className="text-gray-400 font-bold block">හදිසි ඇමතුම් අංකය (Spouse/Emergency):</span>
                    {viewingMother.emergencyPhone || viewingMother.husbandPhone ? (
                      <a href={`tel:${viewingMother.emergencyPhone || viewingMother.husbandPhone}`} className="font-bold text-rose-600 hover:underline flex items-center gap-1">
                        🚨 {viewingMother.emergencyPhone || viewingMother.husbandPhone}
                      </a>
                    ) : <span className="text-gray-400">නොදක්වා ඇත</span>}
                  </div>
                  <div className="sm:col-span-2">
                    <span className="text-gray-400 font-bold block">නිවසේ ලිපිනය (Residential Address):</span>
                    <span className="font-semibold text-gray-700">{viewingMother.address || 'ලිපිනය ඇතුළත් කර නැත'}</span>
                  </div>
                </div>
              </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-gray-100 text-center animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-xl font-black text-gray-800 tracking-tight">මවගේ දත්ත පද්ධතියෙන් ඉවත් කරන්නද?</h3>
            ...
            <div className="bg-gray-50 p-3.5 rounded-2xl my-4 text-left border border-gray-100 text-xs">
              ...
              <div className="text-gray-500 font-mono text-[11px] mt-0.5">NIC: {showDeleteModal.nic || 'නොදක්වා ඇත'} | MOH: {showDeleteModal.mohArea || '—'}</div>
            </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 bg-gradient-to-r from-rose-900 via-pink-900 to-slate-900 text-white p-8 rounded-3xl shadow-xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-96 h-96 bg-rose-500/10 rounded-full blur-3xl pointer-events-none"></div>
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-500/20 text-rose-300 text-xs font-bold mb-2 border border-rose-400/20">
              <span className="w-2 h-2 rounded-full bg-rose-400 animate-pulse"></span>
              National Maternal Registry & Clinical Oversight
            </div>
            <h1 className="text-3xl font-extrabold tracking-tight">ලියාපදිංචි මව්වරුන් කළමනාකරණය</h1>
            <p className="text-rose-100/80 text-xs font-medium mt-1.5 max-w-2xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div>
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-wider block">මුළු ලියාපදිංචි මව්වරුන්</span>
              <span className="text-2xl sm:text-3xl font-black text-gray-800 mt-1 block">{stats.total}</span>
              <span className="text-[10px] text-gray-500 font-semibold mt-0.5 block">Total Registered</span>
            </div>
            <div className="w-12 h-12 rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center text-2xl shadow-inner">
              🤰
            </div>
          </div>

          <div className="bg-white p-5 rounded-3xl border border-red-100 shadow-sm flex items-center justify-between relative overflow-hidden">
            <div className="absolute top-0 right-0 w-20 h-20 bg-red-500/5 rounded-full blur-xl pointer-events-none"></div>
            <div>
              <span className="text-[10px] font-black text-red-500 uppercase tracking-wider block flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                අධි-අවදානම් මව්වරුන්
              </span>
              <span className="text-2xl sm:text-3xl font-black text-red-600 mt-1 block">{stats.highRisk}</span>
              <span className="text-[10px] text-red-600/80 font-bold mt-0.5 block">
          ...
          <div className="bg-white p-5 rounded-3xl border border-emerald-100 shadow-sm flex items-center justify-between">
            <div>
              <span className="text-[10px] font-black text-emerald-600 uppercase tracking-wider block">සාමාන්‍ය තත්ත්වයේ මව්වරුන්</span>
              <span className="text-2xl sm:text-3xl font-black text-emerald-700 mt-1 block">{stats.normal}</span>
              <span className="text-[10px] text-emerald-600/80 font-semibold mt-0.5 block">Normal Risk Pregnancy</span>
            </div>
          ...
          <div className="bg-white p-5 rounded-3xl border border-blue-100 shadow-sm flex items-center justify-between">
            <div>
              <span className="text-[10px] font-black text-blue-600 uppercase tracking-wider block">ආවරණය වන දිස්ත්‍රික්ක</span>
              <span className="text-2xl sm:text-3xl font-black text-blue-800 mt-1 block">{stats.districtsCount} <span className="text-sm font-bold text-gray-400">/ 26</span></span>
              <span className="text-[10px] text-blue-600/80 font-semibold mt-0.5 block">Districts Active</span>
            </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 pb-4">
            ...
            <div className="flex items-center gap-2">
              <span className="bg-rose-50 text-rose-800 text-xs font-bold px-3.5 py-1.5 rounded-full border border-rose-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Search */}
            <div>
              <label className="block text-[10px] font-black text-gray-500 uppercase mb-1">සොයන්න (Search Name / NIC / Phone / Area)</label>
              <div className="relative">
                <input
                  ...
                  className="w-full p-2.5 pr-8 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-rose-500 focus:bg-white outline-none transition-all"
                />
                {searchTerm && (
                  <button 
            ...
            <div>
              ...
              <select
                ...
                className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold focus:ring-2 focus:ring-rose-500 focus:bg-white outline-none transition-all"
              >
            ...
            <div>
              ...
              <select
                ...
                className={`w-full p-2.5 border rounded-xl text-xs font-bold focus:ring-2 focus:ring-rose-500 focus:bg-white outline-none transition-all ${
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-gray-100">
            <span className="text-[10px] font-black text-gray-400 uppercase mr-1">ත්‍රෛමාසිකය (Trimester):</span>
            {[
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          {loading ? (
            <div className="py-24 text-center space-y-3">
              ...
              <p className="text-rose-600 font-bold text-sm">මව්වරුන්ගේ දත්ත පද්ධතියෙන් ලබාගනිමින් පවතී...</p>
              <p className="text-xs text-gray-400">Loading Nationwide Maternal Records...</p>
            </div>
          ) : (
              ...
              <table className="min-w-full table-auto border-collapse">
                ...
                <tbody className="text-gray-600 text-xs divide-y divide-gray-100">
                  {filteredMothers.length > 0 ? filteredMothers.map((mother) => {
                    ...
                    return (
                      <tr key={mother.id} className="hover:bg-rose-50/20 transition duration-150">
                        {/* Mother Name & NIC */}
                        <td className="py-4 px-5">
                          <div className="flex items-center space-x-3">
                            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-lg font-black shrink-0 ${
                            ...
                            <div>
                              <div className="font-bold text-gray-900 text-sm flex items-center gap-2">
                                ...
                                {mother.bloodGroup && (
                                  <span className="bg-rose-50 text-rose-700 text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border border-rose-100">
                                    {mother.bloodGroup}
                                  </span>
                                )}
                              </div>
                        ...
                        <td className="py-4 px-4 text-center whitespace-nowrap">
                          <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-[10px] font-black uppercase ${
                            mother.riskStatus === 'High-Risk' 
                            ? 'bg-red-100 text-red-600 border border-red-200 shadow-sm animate-pulse' 
                        ...
                        <td className="py-4 px-4">
                          <span className="inline-block bg-slate-100 text-slate-700 px-2.5 py-0.5 rounded-lg text-xs font-semibold border border-slate-200">
                            {mother.serviceArea || mother.phmArea || "කලාපයක් දක්වා නැත"}
                          </span>
                          <div className="text-[11px] text-gray-500 mt-0.5">
                            PHM: <span className="font-medium text-gray-700">{mother.midwifeName || 'නොපවරා ඇත'}</span>
                          </div>
                        </td>
                        ...
                        <td className="py-4 px-4 text-center whitespace-nowrap">
                          <div className="flex items-center justify-center space-x-2">
                            ...
                            <button
                              ...
                              className="px-3 py-1.5 bg-rose-50 text-rose-700 hover:bg-rose-600 hover:text-white rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1"
                            >
                    ...
                    <tr>
                      <td colSpan="6" className="py-20 text-center text-gray-400">
                        ...
                        <div className="text-base font-bold text-gray-700">කිසිදු මවකගේ දත්ත හමු නොවීය.</div>
                        <div className="text-xs text-gray-400 mt-1">No registered mothers matching the filter criteria found.</div>
                        {isFiltered && (
                          <button 
                            onClick={resetFilters}
                            className="mt-3 px-4 py-2 bg-rose-50 text-rose-600 rounded-xl text-xs font-bold hover:bg-rose-100 transition-all"
                          >
